...
| `SWARMCLI_MOUSE` | `off`/`false`/`0`/`no` starts the TUI without mouse capture; anything else leaves it on. Read by `app.ProgramOptions`, which both editions' `main` pass to `tea.NewProgram`; `:mouse` toggles it per session. A terminal handover must go through `view.ExecProcess`/`view.Exec`, not `tea.ExecProcess`, or the mouse stays dead after it | unset (mouse on) |
